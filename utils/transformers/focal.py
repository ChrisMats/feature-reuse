### COPY PASTE FROM: https://github.com/microsoft/Focal-Transformer/blob/main/classification/focal_transformer.py
...
def focal_tiny(patch_size=4, img_size=[224], pretrained=False, **kwargs):
    kwargs['drop_path_rate'] = kwargs['drop_path_rate'] if 'drop_path_rate' in kwargs else 0.2    
    model = FocalTransformer(img_size=img_size[0], patch_size=patch_size,
                     embed_dim=96, depths=[2, 2, 6, 2], num_heads=[3, 6, 12, 24], **kwargs)
    if pretrained:
        checkpoint = download_weights("focal_tiny", patch_size, kwargs['pretrained_type'])
        dif_keys = model.load_state_dict(checkpoint, strict=False)
        if dif_keys[0]:
            raise RuntimeError(f"Unexpected key(s) in state_dict: {dif_keys[0]}")   
    return model


def focal_small(patch_size=4, img_size=[224], pretrained=False, **kwargs):
    ...
    if pretrained:
        checkpoint = download_weights("focal_small", patch_size, kwargs['pretrained_type'])
        dif_keys = model.load_state_dict(checkpoint, strict=False)
        if dif_keys[0]:
            raise RuntimeError(f"Unexpected key(s) in state_dict: {dif_keys[0]}") 
    return model
...
def focal_base(patch_size=4, img_size=[224], pretrained=False, **kwargs):
    ...
    if pretrained:
        checkpoint = download_weights("focal_base", patch_size, kwargs['pretrained_type'])
        dif_keys = model.load_state_dict(checkpoint, strict=False)
        if dif_keys[0]:
            raise RuntimeError(f"Unexpected key(s) in state_dict: {dif_keys[0]}")  
    return model    
